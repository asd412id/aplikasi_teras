...
@section('content')
  <div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h3 mb-4 text-gray-800">Ubah Proyek Drainase</h1>
  </div>
        ...
        <div class="card mb-4">
          ...
          <div class="card-body">
            <div class="row form-group">
              ...
              <div class="col-sm-12">
                <input type="text" name="nama_ruas" class="form-control" value="{{ $data->nama_ruas }}" required>
              </div>
            </div>
            <div class="row form-group">
            ...
            <div class="row form-group">
              ...
              <div class="col-sm-12">
                ...
                <select class="form-control" name="jenis_pekerjaan">
                  @foreach ($jp as $key => $p)
                    <option {{ $data->jenis_pekerjaan==$p?'selected':'' }} value="{{ $p }}">{{ ucwords($p) }}</option>
                  @endforeach
                </select>
              </div>
            </div>
            <div class="row form-group">
              ...
              <div class="col-sm-12">
                ...
                <select class="form-control" name="anggaran">
                  @foreach ($jp as $key => $p)
                    <option {{ $data->anggaran==$p?'selected':'' }} value="{{ $p }}">{{ strtoupper($p) }}</option>
                  @endforeach
                </select>
              </div>
            </div>
